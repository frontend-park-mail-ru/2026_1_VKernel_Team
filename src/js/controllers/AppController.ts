...
//     /**
//      * Получение шаблона по имени с кэшированием
//      */
//     async getTemplate(name: TemplateName | string): Promise<HandlebarsTemplateFunction> {
//         if (!this.templates[name as TemplateName]) {
//             try {
//                 const response = await fetch(`/templates/${name}.hbs`);
//                 if (!response.ok) {
...
//                 }
//                 const source = await response.text();
//                 this.templates[name as TemplateName] = Handlebars.compile(source);
//             } catch (error) {
//                 console.error(`Failed to load template ${name}:`, error);
//                 throw error;
//             }
//         }
//         return this.templates[name as TemplateName];
//     },
...
//     async renderNotFound(): Promise<void> {
//         const app = document.getElementById('app');
//         if (!app) return;
//         const template = await this.getTemplate('not-found');
