...
// announcements/place-an-ad/PlaceAnAdController.ts
import Handlebars from 'handlebars';
import { store } from '@/core/store';
import { uiActions } from '@/actions/uiActions';
import { AppController } from '@/controllers/AppController';
...
export class PlaceAnAdController {
    ...
    private static async restoreDraft(): Promise<void> {
        ...
        if (categorySelect) categorySelect.value = draft.formData.category_id || '';
        ...
        if (conditionSelect && draft.formData.category_characteristics) {
            const condition = draft.formData.category_characteristics.find(
                (c: any) => c.name === 'Состояние'
            );
            if (condition) {
                const option = Array.from(conditionSelect.options).find(
                    opt => opt.text === condition.value
                );
                if (option) conditionSelect.value = option.value;
            }
        }
        
        if (brandInput && draft.formData.category_characteristics) {
            const brand = draft.formData.category_characteristics.find(
                (c: any) => c.name === 'Производитель'
            );
            if (brand) brandInput.value = brand.value;
        }
    ...
    private static collectFormData(): any {
        ...
        const categoryCharacteristics: Array<{name: string; value: string}> = [];
        ...
        const customCharacteristics: Array<{name: string; value: string}> = [];
    ...
    private static validateForm(data: any, mode: 'publish' | 'draft'): boolean {
        if (mode === 'publish') {
            if (!data.title) {
                uiActions.showError('Введите название объявления');
                return false;
        ...
        try {
            ...
            if (response.ok && result.success) {
                if (mode === 'publish') {
                    ...
                    uiActions.showSuccess('Объявление сохранено в черновики');
                    AppController.navigateTo('/profile/drafts');
                }
            } else {
                this.showFormErrors(result);
                uiActions.showError(result.error || 'Ошибка при сохранении объявления');
